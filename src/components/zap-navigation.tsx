...
export function ZapNavigation() {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between h-[calc(env(safe-area-inset-bottom,0px)+52px)] px-1">
        {navItems.map((item) => {
          if (item.isCenter) {
            return (
              <div key={item.id} className="flex-1 flex justify-center pt-2">
                ...
                >
                  {/* Effet TikTok Plus Button */}
                  <div className="absolute inset-0 bg-[#00f2ea] rounded-lg -left-[3px]" />
                  <div className="absolute inset-0 bg-[#ff0050] rounded-lg -right-[3px]" />
                  <div className={cn(
            ...
            >
              <Icon 
                className={cn(
                  ...
                  !isActive && (isFeedMode ? "opacity-60" : "opacity-40")
                )} 
              />
              <span className={cn(
                "text-[9px] mt-0.5 font-bold transition-all duration-200",
                isActive ? "opacity-100" : (isFeedMode ? "opacity-60" : "opacity-40")
              )}>
                {item.label}
              </span>
            </Link>
