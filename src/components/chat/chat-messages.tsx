...
const MessageFocusView = ({
    ...
    const renderDeleteView = () => (
      ...
      >
        <div className="absolute -inset-16 bg-destructive/20 blur-3xl -z-10" />
        <div className="w-full flex justify-center opacity-50 blur-sm">
           <ChatMessage 
              message={{...message, sender}} 
              position={sender.id === chatContext.loggedInUser.uid ? 'right' : 'left'} 
              onOpenContextMenu={() => {}} 
        ...
        <div className="w-full max-w-xs space-y-3 mt-6">
            ...
            <Button variant="secondary" className="w-full h-12" onClick={() => { onDeleteForMe(); onClose(); }}>
                <Trash2 className="mr-2" />
                Supprimer pour moi
            </Button>
            <Button variant="ghost" className="w-full h-12" onClick={() => setViewMode('main')}>
                Annuler
            </Button>
        </div>
      </motion.div>
    ...
    const renderShareView = () => (
      ...
      >
        <div className="p-4 border-b flex justify-between items-center">
            <Button variant="ghost" size="icon" onClick={() => setViewMode('main')}><ArrowLeft className="w-4 h-4"/></Button>
